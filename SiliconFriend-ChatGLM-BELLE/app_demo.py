...
# data_args.memory_file: 'update_memory_0512_eng.json'
# memory_dir: '../../memories/update_memory_0512_eng.json'
if not os.path.exists(memory_dir):    
    os.makedirs(os.path.dirname(memory_dir))
...
if model_args.model_type=='chatglm': # model 和 tokenizer 在这个地方被load进来
...
def chat(model, tokenizer, query: str, history: List[Tuple[str, str]] = None, max_length: int = 2048, num_beams=1,
    ...
    print("当前的提示词：\n",prompt)
    inputs = tokenizer([prompt], return_tensors="pt") # tokenizer 编码
    inputs = inputs.to(model.device)
    outputs = model.generate(**inputs, **gen_kwargs)
    outputs = outputs.tolist()[0][len(inputs["input_ids"][0]):]
    response = tokenizer.decode(outputs) # tokenizer 解码
...
def predict_new(
    ...
    # print(history)
    response = chat(model,tokenizer,text,history=history, # 在Demo中对话时会调用这个函数
                    num_beams=1, 
                    top_p=top_p,
...
def retry(
    ...
    if len(history) == 0:
        ...
        return
    # chatbot.pop()
    # inputs = history.pop()[0] # 这里原本的代码有问题
    chatbot.pop()
...
if __name__=="__main__":
    ...
    with gr.Blocks(css=customCSS, theme=small_and_beautiful_theme) as demo:
        ...
        # memory_dir = gr.State("")
        user_memory_index = gr.State(None) # 跟索引记忆有关
        with gr.Row():
            gr.HTML(title)
        ...
        update_memory_args = dict(
            ...
            inputs=[data_args,memory,user_name], # 传参必须传的是gr.State，估计是没法直接传model过去
            outputs=[user_memory],
            show_progress=True
        )
        predict_args = dict(
        ...
        UpdateMemoryBtn.click(**update_memory_args) # 进行UI界面上button的功能 这一个button使来总结用户性格和历史事件的
